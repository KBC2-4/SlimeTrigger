		...
		if (playerState == PLAYER_MOVE_STATE::HOOK) {
			...
			if (moveType == 0) {
				DrawRotaGraph3F(hookX + nX + stage->GetScrollX(), hookY + nY + stage->GetScrollY(), 80, 80,
					...
					images[3][1], TRUE, moveType);
			}
			else {
				DrawRotaGraph3F(hookX + nX + stage->GetScrollX(), hookY + nY + stage->GetScrollY(), 80, 80,
					(distance) / MAP_CEllSIZE / 2, 0.6f, (double)angle,
					images[3][0], TRUE, moveType);
			}
		}
		//伸びる時
		else {
...
void PLAYER::Move()
{
	...
	if (input_lx < -DEVIATION || input_lx > DEVIATION)
	{
		...
		{
			if (jumpMoveX == 0) jumpMoveX = moveX;
			moveType = (jumpMoveX > 0) ? 0 : 1;
			if (jumpMode == 1) //停止ジャンプだった時
			{
				playerX += jumpMoveX * playerSpeed / 2.0f;
			}
			else //移動ジャンプだった時
			{
				moveType = (jumpMoveX > 0) ? 0 : 1;
	...
	else
	{
		...
		if (animation[move].type > 1 && animationState == PLAYER_ANIM_STATE::MOVE)
		{
...
void PLAYER::JumpMove() {
	...
	//落下中
		...
		if (animationState == PLAYER_ANIM_STATE::LANDING) {
			if (animation[static_cast<int>(animationState)].endAnim) {
				ChangeAnimation(PLAYER_ANIM_STATE::IDLE);
			}
		}
	}
...
void PLAYER::MoveAnimation() {
	...
	if (++animation[type].frame % animation[type].switchFrame == 0) {
		animation[type].frame = 0;
		//前半のアニメーション
		if (animation[type].phase == 0 && animation[type].type < animation[type].imageNum - 1) {
			animation[type].type++;
		}
		//後半のアニメーション
		else {
			if (animation[type].playType == 0) {
				animation[type].type--;
			}
			else if (animation[type].playType == 1) {
				//animation[type].phase = 0;
				animation[type].type = 0;
			}
		}
		//前半と後半の切り替え
		if (animation[type].type >= animation[type].imageNum - 1 || animation[type].type <= 0) {
			if (animation[type].playType == 0) {
